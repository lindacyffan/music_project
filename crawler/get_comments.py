...
fieldnames = ['id','comments_content','comments_likedCount','time']
...
for song in reader:
    ...
    for comment in data['hotComments'][:3]:
        row={'id':id,'comments_content':comment['content'],'comments_likedCount':comment['likedCount'],'time':comment['time']}
        writer.writerow(row)
    time.sleep(0.5)
file.close()
f.close()